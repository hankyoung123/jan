...
Via Jan:
1. Settings (<Settings width={16} height={16} style={{display:"inline"}}/>) > General > App Logs
2. Click **Open Logs**

![Open Jan Data Folder](./_assets/server-troubleshooting.png)
...
# Linux (default — resolves via XDG_DATA_HOME)
cd ~/.local/share/Jan/data
```

<Callout type="info">
The data folder location can be customised from **Settings > General > Jan Data Folder**. Once changed, the path above no longer applies — check the Settings page for the active location.
</Callout>

## Directory Structure

Inside the data folder you will typically find:
...
/llamacpp/
  router.preset.ini
  /backends/<version>/<backend>/    # downloaded llama.cpp builds
  /models/
    <org>/<repo>/
      model.yml
      model.gguf
...
### `logs/`
Debugging headquarters (`/logs/app.log`):
- **[APP]**: Core logs
- **[SERVER]**: API drama
- **[SPECS]**: Hardware confessions

### `llamacpp/models/` and `mlx/models/`
Installed models, organised per engine. Each model has its own `model.yml`:
