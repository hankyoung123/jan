import type { NarrativeSourceSummary } from './useManuscript'

export function NarrativeSourcePicker({
  sources,
  selected,
  onSelect,
}: {
  sources: NarrativeSourceSummary[]
  selected: NarrativeSourceSummary | null
  onSelect: (source: NarrativeSourceSummary) => void
}) {
  return (
      ...
      {sources.map((source) => (
        ...
        >
          <span className="flex items-center justify-between gap-3">
            <strong className="line-clamp-1 text-sm font-medium">{source.title_hint}</strong>
