...
export function useSimulationSession(projectId?: string) {
  ...
  const advanceCommands = useRef(new Map<string, SimulationAdvanceRequest>())
  ...
  useEffect(() => {
    advanceCommands.current.clear()
  }, [sessionId])

  const commandFor = useCallback(
    (operation: 'step' | 'run' | 'resume', snapshot: SessionSnapshot) => {
      const key = `${snapshot.session_id}:${operation}:${snapshot.state_hash}`
      let command = advanceCommands.current.get(key)
      if (!command) {
        command = {
          command_id: `command:${crypto.randomUUID()}`,
          expected_state_hash: snapshot.state_hash,
        }
        advanceCommands.current.set(key, command)
      }
      return { command, key }
    },
    []
  ...
  const performAdvance = useCallback(
    async <T,>(
      ...
      try {
        ...
        advanceCommands.current.delete(key)
        return result
      } catch (cause) {
        // HTTP failures are definitive. Network failures keep the command ID so a
        // retry can retrieve the server's durable receipt instead of advancing twice.
  ...
  const step = useCallback(async () => {
    ...
    const { command, key } = commandFor('step', session)
    const result = await performAdvance('step', key, () =>
    ...
    return result
  }, [commandFor, performAdvance, projectId, refreshSession, session])

  const control = useCallback(
    async (action: 'run' | 'pause' | 'resume' | 'terminate' | 'cancel') => {
      if (!projectId || !session) return null
      const advance = action === 'run' || action === 'resume'
        ? commandFor(action, session)
        : null
      const request = () =>
          ...
          `/projects/${projectId}/simulations/${session.session_id}/${action}`,
          {
            ...
            body:
              action === 'terminate' || action === 'cancel'
                ...
                : advance
                  ? JSON.stringify(advance.command)
                  : undefined,
          }
        )
      const updated = advance
        ? await performAdvance(action, advance.key, request)
        : await perform(action, request)
      if (updated) {
        setSession(updated)
        syncUrl(updated)
      }
    ...
    [commandFor, perform, performAdvance, projectId, session, syncUrl]
  )
