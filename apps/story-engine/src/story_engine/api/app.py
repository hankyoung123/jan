...
def create_app(
    ...
    *,
    ...
    async def lifespan(_app: FastAPI) -> AsyncIterator[None]:
        if runtime_settings.projects_root.exists():
            for root in runtime_settings.projects_root.iterdir():
                if root.is_dir() and (root / "project.md").is_file():
                    simulation_service.recover_projections(root.name)
        yield
    ...
    simulation_service = SimulationApplicationService(
        ...
        receipt_store_factory=lambda project_id: CommandReceiptStore(
            runtime_settings.projects_root / project_id
        ),
    )
