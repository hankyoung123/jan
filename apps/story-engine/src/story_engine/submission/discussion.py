...
class SubmissionDiscussionService:
    ...
    async def respond(
        ...
        task_context = (
            ...
            "severity, "
            "or runnable fields. Preserve supplied character_ref values (c0, c1) and "
            "fact_ref values (f0, f1) when editing existing items; use character_ref "
            "inside fact known_by and fact_ref inside supersedes_ref. Public facts "
            "require an empty known_by array; private or secret facts require every "
            "knowing character ref. The local runtime generates system IDs and both "
            "directions of knowledge links. Every character requires a stable "
            "display_name; never use pronouns such as 你, User, or human as a "
            "display_name. In facts, relationships, identities, goals, and scene "
            "narration, refer to characters by display_name. Natural pronouns are "
            "allowed inside dialogue only. "
