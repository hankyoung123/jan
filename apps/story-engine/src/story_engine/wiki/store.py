...
- semantic interpretation and page-content proposals;
- source references and uncertainty labels;
- no paths, indexes, logs, revisions, hashes, or system identifiers.

Store owns:

- page creation, paths, branch and directory indexes;
- maintenance logs, revisions, hashes, and atomic writes;
- validating that patches stay below `world/` or one subject under `characters/`.

The runtime log, checkpoints, raw memory, `index.md`, and `log.md` remain
Store-owned and are never model-authored.
"""
