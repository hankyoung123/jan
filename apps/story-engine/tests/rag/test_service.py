...
def _project(tmp_path: Path) -> Path:
    ...
    events.append(
        StoryEvent(
            ...
            public_results=("北侧防波堤暂时稳定",),
            hidden_results=("北侧暗门密码为7341",),
...
def test_markdown_index_supports_exact_id_and_bm25_with_citations(
    ...
    assert ranked.hits[0].source_id == "event-000001"
    assert all(
        hit.chunk_id
        and hit.source_type
        and hit.source_id
...
def test_character_scope_filters_unauthorized_markdown_before_scoring(
    ...
    assert {hit.source_id for hit in lin_experienced.hits} == {"event-000002"}
