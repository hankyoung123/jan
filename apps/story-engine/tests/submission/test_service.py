...
def test_submission_stable_refs_preserve_identity_across_renames_and_reordering(
        ...
        SubmissionDraftDelta(
            characters=(
                SubmissionCharacterProposal(
                    character_ref="c1",
                    display_name="林岚 (港务所)",
                    identity=draft.characters[1].identity,
                    core_desire=draft.characters[1].core_desire,
                    current_goal=draft.characters[1].current_goal,
                    location=draft.characters[1].location,
                ),
                SubmissionCharacterProposal(
                    character_ref="c0",
                    display_name="陈默 (灯塔)",
                    identity=draft.characters[0].identity,
                    core_desire=draft.characters[0].core_desire,
                    current_goal=draft.characters[0].current_goal,
                    location=draft.characters[0].location,
                ),
            ),
            facts=(
                SubmissionFactProposal(
                    fact_ref="f2",
                    statement="陈默的父亲在灯塔附近失踪, 线索仍未公开。",
                    visibility="secret",
                    known_by=("c0",),
                ),
            ),
    ...
    assert updated.characters[0].id == draft.characters[1].id
    assert updated.characters[1].id == draft.characters[0].id
    assert updated.facts[0].id == draft.facts[2].id
    assert updated.facts[0].known_by == (draft.characters[0].id,)
