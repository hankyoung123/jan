...
//! (Story Engine CLI) can read the same file without an `AppHandle`. Secrets never land
...
//! synchronous [`flush_settings`] drains the debounce on app exit so Story Engine CLI
...
/// debounce window can't drop the last writes or strand Story Engine CLI with a stale
